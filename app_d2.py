...
OPENROUTER_API_KEY = (
    get_setting("OPENROUTER_API_KEY") or ""
).strip() or None
...
@st.cache_resource(show_spinner=False)
def load_chat_agent() -> FunctionAgent:
    if not OPENROUTER_API_KEY:
        raise RuntimeError(
            "OPENROUTER_API_KEY is not configured."
        )
    ...
    return FunctionAgent(
        ...
        system_prompt=(
            ...
            "before answering factual questions. Use only the exact "
            "tool names provided in your tool schema; never invent or print "
            "XML-style tool-call tags. The deterministic tools are the "
            "source of truth. Preserve candidate IDs "
...
def deterministic_comparison_fallback(
    prompt: str,
) -> tuple[str, list[dict[str, Any]]] | None:
    """
    Return a grounded comparison when a free model fails to emit
    ...
    first = record_lookup.get(candidate_ids[0], records[0])
    second = record_lookup.get(candidate_ids[1], records[1])

    first_id = str(first.get("candidate_id", candidate_ids[0]))
    second_id = str(second.get("candidate_id", candidate_ids[1]))

    rows = [
        "| Metric | "
        f"{first_id} | {second_id} | Stronger |",
        "|---|---:|---:|---|",
    ]
    ...
    if wins[first_id] > wins[second_id]:
        conclusion = (
            f"**Conclusion:** {first_id} is stronger on "
            f"{wins[first_id]} of the five requested category scores."
        )
    elif wins[second_id] > wins[first_id]:
        conclusion = (
            f"**Conclusion:** {second_id} is stronger on "
            f"{wins[second_id]} of the five requested category scores."
        )
    else:
        conclusion = (
            "**Conclusion:** The two candidates split the five "
            "requested category-score comparisons."
        )
    ...
    response_parts = [
        ...
        *rows,
        ...
        (
            ...
            "It is planning screening, not confirmed electrical, "
            "construction, ownership, or permitting feasibility."
        ),
    ]
    ...
    trace = [
        {
            ...
            "details": {
                ...
                "metrics": metrics,
                "reason": (
                    "The selected free model did not return a valid "
...
async def run_agent_async(
    ...
    pseudo_tool_call = (
        ...
        or "<arg_key>" in response_text
        or "get_candidate_details" in response_text
    )
    ...
    if (
        pseudo_tool_call
        or malformed_markers
        or (
            "compar" in prompt.lower()
            and real_tool_call_count == 0
        )
    ):
        fallback = deterministic_comparison_fallback(
            prompt
        )
        ...
        raise RuntimeError(
            "The selected free model did not return a valid "
            "structured tool call. Try another fixed free model."
        )
